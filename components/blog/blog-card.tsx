...
export const BlogCard = ({article, index, filter}: BlogCardProps) => {
  return (
    <Link
      className={`article-card group cursor-pointer flex flex-col ${index === 0 && filter === "All" ? "md:col-span-2 lg:col-span-3 lg:flex-row gap-12" : ""}`}
    ...
    >
      ...
      <div className={`flex flex-col justify-center ${index === 0 && filter === "All" ? "lg:w-1/3" : ""}`}>
        ...
        <h3 className={`font-display font-bold text-white mb-4 leading-tight group-hover:text-cyan-400 transition-colors ${index === 0 && filter === "All" ? "text-4xl md:text-5xl" : "text-xl md:text-2xl"}`}>
          {article.title}
        </h3>
